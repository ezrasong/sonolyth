...
class MetadataPluginScrobbleNotifier
    extends Notifier<StreamController<SonolythTrackObject>?> {
  @override
  build() {
    ...
    final controller = StreamController<SonolythTrackObject>.broadcast();
    ...
    final subscription = controller.stream.listen((event) async {
      try {
        await metadataPlugin.valueOrNull?.core.scrobble({
          ...
          "isrc": event is SonolythFullTrackObject ? event.isrc : null,
        });
      } catch (e, stack) {
...
final metadataPluginScrobbleProvider = NotifierProvider<
    MetadataPluginScrobbleNotifier, StreamController<SonolythTrackObject>?>(
